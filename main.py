...
def define_env(env):
    ...
    env.variables["tenet3"] = tenet3_adm(
        ...
        icon="var(--inline-icon--flag)",
    )
    env.variables["tenet4"] = tenet4_adm(
        ...
        icon="var(--inline-icon--shield-heart)",
    )
    env.variables["tenet5"] = tenet5_adm(
        ...
        icon="var(--inline-icon--shield-cross)",
    )
    env.variables["tenet6"] = tenet6_adm(
    ...
    env.variables["tenet7"] = tenet7_adm(
    ...
    env.variables["tenet6_faust2024"] = tenet6_adm(
        "SLA points awarded from recovering services do not scale with amount of uptime in the *retention period*.",
        classnames="warning",
        open=True,
    )
    env.variables["tenet7_faust2024"] = tenet7_adm(
        "Flag value is not scaled to the number of flagstores and thus independent.",
        classnames="success",
    )

    env.variables["tenet1_saar2024"] = tenet1_adm(
        "Attack points scale linearly with the amount of flags captured.",
        classnames="success",
    )
    env.variables["tenet2_saar2024"] = tenet2_adm(
        "Defense points scale linearly with the amount of flags lost.",
        classnames="success",
    )
    env.variables["tenet3_saar2024"] = tenet3_adm(
        "Flag values scales inversely with the amount of captures.",
        classnames="success",
    )
    env.variables["tenet4_saar2024"] = tenet4_adm(
        "Depending on the number of flagstores, more points can be lost from defense than are gained from SLA.",
        classnames="failure",
        open=True,
    )
    env.variables["tenet5_saar2024"] = tenet5_adm(
        "The cost of downtime is similar to the cost of defense per round. Patches prevent loss of points over multiple rounds and are thus favorable to not patching.",
        classnames="success",
    )
    env.variables["tenet6_saar2024"] = tenet6_adm(
        "SLA points awarded from recovering services do not scale with amount of uptime in the *retention period*.",
        classnames="warning",
        open=True,
    )
    env.variables["tenet7_saar2024"] = tenet7_adm(
        "SLA points are typically not significantly larger than defense points.",
        classnames="success",
    )

    env.variables["tenet1_enowars2024"] = tenet1_adm(
        "Attack points scale linearly with the amount of flags captured.",
        classnames="success",
    )
    env.variables["tenet2_enowars2024"] = tenet2_adm(
    ...
    env.variables["tenet3_enowars2024"] = tenet3_adm(
        "Flag values scales inversely with the amount of captures.",
        classnames="success",
    )
    env.variables["tenet4_enowars2024"] = tenet4_adm(
    ...
    env.variables["tenet6_ecsc2023"] = tenet6_adm(
        "SLA does not decrease fairly with the amount of missing flags in the retention period.",
        classnames="success",
    )
    env.variables["tenet7_ecsc2023"] = tenet7_adm(
        "Flag value is not scaled to the amount of flagstores, and thus independent of flagstore.",
        classnames="success",
    )

    env.variables["tenet1_ecsc2024"] = tenet1_adm(
        "Attack points scale linearly with the amount of flags captured.",
        classnames="success",
    )
    env.variables["tenet2_ecsc2024"] = tenet2_adm(
        "Defense points scale linearly with the amount of flags lost",
        classnames="success",
    )
    env.variables["tenet3_ecsc2024"] = tenet3_adm(
    ...
    env.variables["tenet4_ecsc2024"] = tenet4_adm(
    ...
    env.variables["tenet3_ecsc2025"] = tenet3_adm(
        "Each flag's value is inversely proportional to the number of captures.",
        classnames="success",
    )
    env.variables["tenet4_ecsc2025"] = tenet4_adm(
    ...
    env.variables["tenet5_ecsc2025"] = tenet5_adm(
    ...
    env.variables["tenet6_ecsc2025"] = tenet6_adm(
        "Teams only lose SLA and DEF points relative to the amount of rounds each flag has been made unavaiable for.",
        classnames="success",
    )
    env.variables["tenet7_ecsc2025"] = tenet7_adm(
        "Flag value does not depend on the amount of flagstores in the service.",
        classnames="success",
    )

    env.variables["tenet1_atklabv1"] = tenet1_adm(
        "Score increases with attack, which scales with flags captured.",
        classnames="success",
    )
    env.variables["tenet2_atklabv1"] = tenet2_adm(
        "Score decreases with defense, which scales with flags lost.",
        classnames="success",
    )
    env.variables["tenet3_atklabv1"] = tenet3_adm(
        "A flag's value scales inversely with the number of captures.",
        classnames="success",
    )
    env.variables["tenet4_atklabv1"] = tenet4_adm(
        "The maximum points gained by any attack (`flagstores * 2`) is less than the minimum cost of downtime (`sla_max = flagstores * 2 + 1`).",
        classnames="success",
    )
    env.variables["tenet5_atklabv1"] = tenet5_adm(
        "The cost of downtime due to patching can be recovered in few subsequent rounds of prevented exploitation.",
        classnames="success",
    )
    env.variables["tenet6_atklabv1"] = tenet6_adm(
        "`sla_ratio` decreases fairly with every missing flag in the *validity period*.",
        classnames="success",
    )
    env.variables["tenet7_atklabv1"] = tenet7_adm(
        "Flag value does not depend on the amount of flagstores in the service.",
        classnames="success",
    )

    env.variables["tenet1_atklabv2"] = tenet1_adm(
        "Attack points scale linearly with the amount of flags submitted.",
        classnames="success",
    )
    env.variables["tenet2_atklabv2"] = tenet2_adm(
        "Defense points are awarded for rounds a team is not exploited, despite another team successfully exploiting that flagstore, proportional to the amount of flags available. Thus teams who have not patched lose more flags the more flagstores and flags from those flagstores are captured (in relation to a patched team).",
        classnames="success",
    )
    env.variables["tenet3_atklabv2"] = tenet3_adm(
        "Each flag's value is inversely proportional to the number of captures.",
        classnames="success",
    )
    env.variables["tenet4_atklabv2"] = tenet4_adm(
        "An attacker's marginal gain per service per team is `flagstores * base`, the same as the minimum gain for the defender from keeping it alive. Therefore, no points gained from turning off a service, especially not in regards to the other non-attacking players.",
        classnames="success",
    )
    env.variables["tenet5_atklabv2"] = tenet5_adm(
        "The cost of downtime due to patching can be recovered in few subsequent rounds through defense points gained (see calculation in review section).",
        classnames="success",
    )
    env.variables["tenet6_atklabv2"] = tenet6_adm(
        "Teams only lose SLA and DEF points relative to the amount of rounds each flag has been made unavaiable for.",
        classnames="success",
    )
    env.variables["tenet7_atklabv2"] = tenet7_adm(
        "Flag value does not depend on the amount of flagstores in the service.",
        classnames="success",
    )
