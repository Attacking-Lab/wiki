...
We determine a flexible dynamic scoring formula anchored at three exact fixed
points: `(1, max_score)`, `(fp_ratio * teams, fp_weight * max_score)` and
`(teams, min_score)`. The endpoints are pinned by the linear normalization
`solve_ratio = (solves - 1) / (teams - 1)`, while `mid_ratio` (the value of
`solve_ratio` at the middle anchor `fp_ratio * teams`) is used as the base of
`rate` so that the middle point lands on `fp_weight * max_score`.
...
def jeopardy(solves: int, teams: int,
    ...
    mid_ratio   = (fp_ratio * teams - 1) / (teams - 1)
    solve_ratio = (max(1, solves) - 1) / (teams - 1)
    rate = log(log(fp_weight, score_ratio), mid_ratio)
...
??? "Implementation Details"
    ...
    - `max_score`: The maximum score, awarded when the number of solves is one.
    - `min_score`: The minimum score, awarded when the number of solves equals the number of teams.
    - `fp_ratio`: The fraction of teams at which the middle fixed point is placed (`fp_ratio * teams` solves).
    - `fp_weight`: The fraction of `max_score` awarded at the middle fixed point.
...
def attack(live_round: int, flag_round: int,
           max_victims: int, num_victims: int, num_attackers: int, teams: int):
    ...
    pts = defense(live_round, flag_round, checker_status, flag_avail_in,
                  max_victims, num_victims, num_attackers, False)
    for flag in flags_stolen:
        pts += attack_flag(flag.num_submissions, teams)
    return pts
```

??? "Implementation Details"

    **Context**: Each <span class=hltext>round</span>, this function is called per <span class=hltext>attack</span>, for the <span class=hltext>service</span> and <span class=hltext>flagstore</span> being attacked by an <span class=hltext>attacker</span>, to calculate the value of the entire attack over all victims.
...
def defense(live_round: int, flag_round: int, checker_status: dict[int, str],
    ...
    pts = 0
    max_round = min(live_round + 1, flag_round + flag_rounds_valid)
...
??? "Implementation Details"
    ...
    **Context**: Each <span class=hltext>round</span>, this function is called per <span class=hltext>service</span> and <span class=hltext>flag store</span>, for each active <span class=hltext>attacker</span> and for every <span class=hltext>team</span>, to update the value of teams defending / not defending the <span class=hltext>attack</span>.
    ...
    - `flag_avail_in`: A mapping for which flags were retrievable from a specific round (first key), depending on the team, service, flagstore and round they were deployed in. **Remember:** each round the checker checks that valid flags can be retrieved.
    - `max_victims`: The number of teams who are not the attacker or NOP, that have atleast one service not in <span class=hl-offline>`OFFLINE`</span> state in the current round.
    - `num_victims`: The number of teams exploited by the attack which points are currently being calculated for.
    - `num_attackers`: The number of teams attacking this <span class=hltext>service</span> and <span class=hltext>flagstore</span>, and obtaining flags stored in <span class=hltext>flag_round</span>.
...
```python3
def sla(checker_status: str, flags_avail: int, flagstores: int,
        max_score: int = 10, flag_rounds_valid: int = 5):
    if checker_status == "SUCCESS":
        # all flagstores * flag_rounds_valid flags retrievable, so this is
        # equivalent to the RECOVERING branch at full availability
...
??? "Implementation Details"
    ...
    **Context**: Each <span class=hltext>round</span>, this function is called per <span class=hltext>team</span> and per <span class=hltext>service</span>.
    ...
    - `flagstores`: The number of flag stores in <span class=hltext>service</span>.
    - `max_score`: The maximum value of each *challenge*, see `jeopardy(..)` definition.
    - `flag_rounds_valid`: The number of rounds each flag is valid for.

This means that at the start of the CTF, SLA points ramp up from zero to
`max_score * flagstores` over the first five rounds, as the validity period is
five rounds long.
...
- Teams only lose SLA and DEF points relative to the amount of rounds each flag
  has been made unavailable for. Since the points from defending are spread across the
  rounds in which the flag is being checked, and recovering points are awarded
  according to the amount of flags available, patching does not cost the
...
- A service should be unavailable for at most a few (let's say 3) rounds due
  to (unsuccessful) patching. Let `base = max_score` be the per-flag base value.
  We find that the points lost are
  ...
  the additional patched flagstore is `(teams - 2) * jeopardy(teams - 2, teams)` per round,
