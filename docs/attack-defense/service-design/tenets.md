...
{% set service_tenet1 = admon("A service **MUST** be able to keep track of flags for a specified number of rounds", "The service **must** keep track of flags for atleast one round to allow extraction through exploitation. Requiring flags to be stored for more than one round allows for use of [recovery periods](/attack-defense/scoring/faust2024.md) and for players to investigate service artifacts related to network traffic before they are removed.", icon="var(--inline-icon--shield-heart)") %}
